...
function renderHub() {
    ...
    document.getElementById('btn-share-folder').addEventListener('click', async () => {
        if (!IS_TAURI) {
            showToast('Solo disponible en desktop');
            return;
        }
        try {
            const item = await invoke('share_folder');
            if (item) {
    ...
    document.getElementById('btn-share-all').addEventListener('click', async () => {
        ...
        if (allPublished) {
            ...
            try {
                ...
                ids.forEach(id => publishedIds.delete(id));
                renderLocalContent();
                showToast(`Error: ${e instanceof Error ? e.message : e}`);
            }
        }
...
function attachDragScroll(el) {
    ...
    el.addEventListener('mousedown', (e) => {
        if (e.target.closest('button, a, input, .content-card'))
            return;
        isDown = true;
...
function renderLocalContent() {
    ...
    container.querySelectorAll('[data-action]').forEach(btn => {
        btn.addEventListener('click', async (e) => {
            ...
            else if (action === 'broadcast' || action === 'direct')
                publishedIds.add(id);
            renderLocalContent();
            try {
                if (action === 'stop') {
    ...
    container.querySelectorAll('.content-card').forEach(card => {
        ...
        if (id && item && item.content_type !== 'text') {
            ...
            cardEl.addEventListener('pointermove', (event) => {
                if (!dragStartPoint || nativeDragStarted)
                    return;
                ...
                setTimeout(() => {
                    if (cardEl.dataset.suppressClick === '1')
                        delete cardEl.dataset.suppressClick;
                }, 800);
...
function isPeerUnreachableError(msg) {
    const lower = msg.toLowerCase();
    return (lower.includes('peer content not found') ||
...
function startPeerSync() {
    if (peerSyncInterval !== null)
        return;
    peerSyncInterval = setInterval(async () => {
        if (!IS_TAURI || document.visibilityState !== 'visible')
            return;
        try {
            const freshPeers = await invoke('get_peers');
...
function iconFolder(s) { return svg(s, '0 0 16 16', '<path d="M1.5,4.5 h5 l1.5,2 h6 a1,1 0 0 1 1,1 v5 a1,1 0 0 1 -1,1 h-12 a1,1 0 0 1 -1,-1 v-7 a1,1 0 0 1 1,-1 z" stroke-width="1.8"/>'); }
...
function contentVisualKind(item) {
    if (item.content_type === 'text')
        return 'text';
    ...
    if (mime.startsWith('audio/') || ['wav', 'wave', 'mp3', 'm4a', 'aac', 'flac', 'ogg', 'oga', 'opus', 'wma', 'aif', 'aiff', 'mid', 'midi'].includes(ext))
        return 'audio';
    if (mime.startsWith('video/') || ['mp4', 'mov', 'mkv', 'webm', 'avi', 'wmv', 'm4v', 'mpeg', 'mpg'].includes(ext))
        return 'video';
    if (mime === 'application/pdf' || ext === 'pdf')
        return 'pdf';
    if (['zip', 'rar', '7z', 'tar', 'gz', 'tgz', 'bz2', 'xz'].includes(ext))
        return 'archive';
    if (['js', 'ts', 'tsx', 'jsx', 'rs', 'py', 'go', 'java', 'kt', 'swift', 'c', 'cpp', 'h', 'hpp', 'cs', 'sh', 'zsh', 'json', 'toml', 'yaml', 'yml', 'md', 'html', 'css', 'sql'].includes(ext))
        return 'code';
    if (mime.includes('spreadsheet') || mime.includes('excel') || ['xls', 'xlsx', 'csv', 'ods'].includes(ext))
        return 'spreadsheet';
    if (mime.includes('word') || mime.includes('document') || ['doc', 'docx', 'odt', 'rtf'].includes(ext))
        return 'document';
    return 'file';
}
